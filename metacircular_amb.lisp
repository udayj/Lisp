...
;
(defparameter *evaluator-debug-level* 0)
...
(defun eval. (exp env)
  ...
  (cond ((self-evaluating? exp) 
        ...
        (t
          (error "Unknown expression in EVAL: "))))
...
(defun apply. (proc args)
  ...
  (cond ((primitive-procedure? proc)
        ...
        (t
          (error
            "Unknown procedure type in APPLY: " ))))
...
(defparameter the-global-environment (setup-env))
  ...
  (if (null aprocs)
      ...
      (funcall (car aprocs)
		 ...
		 (get-args (cdr aprocs)
			   env
			   (lambda (args fail3)
			     (funcall succeed (cons arg args)
				      fail3))
			   fail2))
	       fail)))
(defun execute-application (proc args succeed fail)
  (cond ((primitive-procedure? proc)
	 (funcall succeed (apply-primitive-procedure proc args) fail))
	((compound-procedure? proc)
	 (funcall
	  (procedure-body proc)
	  (extend-environment 
